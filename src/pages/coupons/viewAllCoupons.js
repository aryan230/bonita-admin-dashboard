...
function createData(title, key, code, type, edit, deleteCat) {
  return { title, key, code, type, edit, deleteCat };
}
...
const AllCoupons = () => {
  ...
  useEffect(async () => {
    ...
    try {
      const response =
        await axios.get(`https://postman365.herokuapp.com/api/coupon
      `);
